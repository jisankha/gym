...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT r.*,p.plan,p.amount as pamount,concat(m.lastname,', ',m.firstname,' ',m.middlename) as name,m.member_id as mid_no from registration_info r inner join members m on m.id = r.member_id inner join plans p on p.id = r.plan_id where r.id=".$_GET['id'])->fetch_array();
	foreach($qry as $k =>$v){
		$$k = $v;
	}
...
<div class="container-fluid">
	...
	<hr class="divider">
	<p>Plan Membership Fee: </i> <b><?php echo number_format($pamount,2) ?></b></p>
	<p>Trainer Fee: </i> <b><?php echo number_format($tf) ?></b></p>
	<p>Monthly Payable: </i> <b><?php echo number_format(($pamount + $tf),2) ?></b></p>
</div>
<div class="modal-footer display">
	<div class="row">
		<div class="col-md-12">
			<button class="btn float-right btn-secondary" type="button" data-dismiss="modal">Close</button>
			<button class="btn float-right btn-primary mr-2" type="button" id="payment">View Payments</button>
